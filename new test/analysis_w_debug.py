...
delay_s       = 4.2999854002387226  # clock-corrected carrier delay from delay_estimates.npz
...
DELAYED_SB_KEY = 'delayer_sb'         # delayed sideband -- for the alt residual combo
...
channels = {
    ...
    'sb': {'phase': col(DELAYED_SB_KEY), 'freq': col(f'{DELAYED_SB_KEY}_freq')},
}
...
print(f"Measuring timing jitters from pilot tone '{PT_KEY}'")
...
# delayer_sb is already physically delayed (recorded after the signal passed
# through the delay line) -- do NOT shift it again in software.
...
dcarrier_phase_d = detrend(ch3_phase)  # raw (unshifted) delayer_carrier phase
...
# ── 7b. ALTERNATIVE COMBINATION -- delayer_sb vs delayer_carrier, both raw
...
# frequency *difference* between the two Input-A channels, still referenced
# to the delayer's own pilot tone (delayer_carrier_b). ───────────────────────
tdi_alt = (
    sb_phase_d
    - dcarrier_phase_d
