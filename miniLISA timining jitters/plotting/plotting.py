...
def build_eta_for_noise(noise_type):
    ...
    for (i,j) in tau:
        ...
        t_ij=tau[(i,j)]; Nij,Nji,nij,nji=N[(i,j)]; Bij, BijSB=B[(i,j)]
        ...
        elif noise_type == 'optical':        eta[(i,j)] = expand(Nij(t)-D(Nji(t),t_ij))
...
def build_eta_and_sb_for_noise(noise_type):
    ...
    for (i,j) in tau:
        ...
        elif noise_type == 'optical':        c = expand(Nij(t)-D(Nji(t),t_ij))
        ...
        # ── sideband etaSB ───────────────────────────────────────────────────
        # Always include modulation noise (matches notebook etaSB definition).
        # The correction r = -(eta_C - etaSB)/omm_j then picks up modulation
        # residuals in X1c, which is the physically correct behaviour.
        mod = expand(N_i_m(t) - D(N_j_m(t), t_ij))

        if   noise_type == 'phi':        sb = expand(D(phi_j(t),t_ij)-phi_i(t)) + mod
        elif noise_type == 'clock':      sb = expand(-(om_j-om_i+omm_j-omm_i)*q_i(t)
                                                     - omm_i*q_i(t)
                                                     + omm_j*D(q_j(t),t_ij)) + mod
        elif noise_type == 'board':      sb = expand((om_j+omm_j)*(eps_i(t)-D(eps_i(t),t_ij))) + mod
        elif noise_type == 'optical':    sb = expand(nij(t)-D(nji(t),t_ij)) + mod
        elif noise_type == 'optical_sb': sb = expand(nij(t)-D(nji(t),t_ij))  # no extra mod
        elif noise_type == 'modulation': sb = expand(N_i_m(t)-D(N_j_m(t),t_ij))
        # Note: modulation has eta=0 (set in carrier section above),
        # etaSB = N_i_m - D(N_j_m) across ALL 6 links.
        # Each N_i_m appears in 4 etas (sent on 2 links, received on 2 links),
        # giving the full multi-term H structure in X1c.
...
noise_config = {
    'clock': {
        ...
        'S_func': S_clock, 'divide_2pi': True,
    ...
    'board': {
        ...
        'S_func': S_board, 'divide_2pi': True,
        'ylabel_f': r'ASD [Hz / $\sqrt{\mathrm{Hz}}$]',
        'ylabel_p': r'ASD [cycles / $\sqrt{\mathrm{Hz}}$]',
    ...
    'modulation': {
        ...
        'S_func': S_modulation, 'divide_2pi': False,
        'ylabel_f': r'ASD [cycles$\cdot$Hz / $\sqrt{\mathrm{Hz}}$]',
        'ylabel_p': r'ASD [cycles / $\sqrt{\mathrm{Hz}}$]',
    ...
    'optical': {
        ...
        'S_func': S_optical, 'divide_2pi': False,
        'ylabel_f': r'ASD [cycles$\cdot$Hz / $\sqrt{\mathrm{Hz}}$]',
        'ylabel_p': r'ASD [cycles / $\sqrt{\mathrm{Hz}}$]',
    ...
    'board_baseline': {
        ...
        'S_func': S_board_baseline, 'divide_2pi': False,
        'ylabel_f': r'ASD [cycles$\cdot$Hz / $\sqrt{\mathrm{Hz}}$]',
        'ylabel_p': r'ASD [cycles / $\sqrt{\mathrm{Hz}}$]',
