...
for ch in [1,2,4]:
    x = detrend(channels[ch]['phase'], type='linear')
    channels[ch]['phase_detrended'] = x - x[0]  # make the initial phase zero
...
plt.tight_layout()
plt.savefig(f'plots/first_look_{filename}.pdf', dpi=150)
"""
...
plt.tight_layout()
plt.savefig(f'plots/asd_{filename}.pdf', dpi=150)
"""