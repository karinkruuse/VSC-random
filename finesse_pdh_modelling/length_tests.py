...
plt.plot((f_range - f0) * 1e-6, R1, label="L1 cavity", color="maroon")
plt.plot((f_range - f0) * 1e-6, R2, label="LR cavity", color="gray")
...
for f in fr_peaks:
    plt.axvline((f - f0) * 1e-6, color='gray', linestyle='--', alpha=0.3)
...
plt.axvspan(25, 60, color='gray', alpha=0.5, label="Target range")
