...
instruments -- to find the relative clock offset. corr1 is the reference:
its own native sample times are used as the output grid, untouched. corr2
is shifted onto that grid by the estimated offset (via interpolation, since
...
Output: data/no sb/synced_clocknoise_corr.npz containing:
    time_s              -- common time axis (s), corr1's native grid
...
OUT_NPZ = os.path.join(DATA_DIR, "synced_clocknoise_corr.npz")
OUT_PLOT = os.path.join(os.path.dirname(__file__), "synced_clocknoise_overview.png")
OUT_PLOT_RAW = os.path.join(os.path.dirname(__file__), "unsynced_clocknoise_dds.png")
...
def main():
    ref = load_slot(CORR1_PREFIX, CORR1_CHANNELS, "corr1")
    other = load_slot(CORR2_PREFIX, CORR2_CHANNELS, "corr2")
    slots = [ref, other]

    if ref["rate"] != other["rate"]:
        raise ValueError(f"sample rates differ: corr1={ref['rate']}, corr2={other['rate']}")
    ...
    print(f"Estimating corr1<->corr2 offset from the first {WINDOW_S:.0f} s of the dds channels:")
    ...
    # use corr1's own native sample times as the output grid, cropped to
    # where both slots have data after the offset is applied -- corr1
    # (offset 0 by definition) is never resampled/touched, only corr2 gets
    # shifted (via apply_offset) onto corr1's grid
    ...
    # on every channel including corr1's (which wasn't itself shifted, but
    ...
    # verify the thing that actually matters: that the corr1<->corr2 offset
