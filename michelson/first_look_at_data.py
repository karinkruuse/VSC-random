...
if CROP_FRACTION < 1.0:
    ...
    print(f"Cropped to {CROP_FRACTION*100:.1f}% of record: {len(data)} samples (M={M})")
...
for ch in range(1, nr_of_channels + 1):
    ...
    channels[ch]['amplitude'] = np.hypot(channels[ch]['I'], channels[ch]['Q'])
...
fig, axes = plt.subplots(nr_of_channels, 3, figsize=(20, 10))
...
for i, ch in enumerate(range(1, nr_of_channels + 1)):
    ...
    ax_a = axes[i, 2]
    ax_a.plot(t_abs[M:N], channels[ch]['amplitude'][M:N], lw=0.6, label='sqrt(I^2+Q^2)')
    ax_a.set_ylabel(f'Ch{ch} amplitude (V)')
